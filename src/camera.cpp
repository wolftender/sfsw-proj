...
namespace mini {
	...
	ortho_camera::ortho_camera() {
		m_left = m_top = -5.0f;
		m_right = m_bottom = 5.0f;
		m_near = 0.1f;
		m_far = 100.0f;
		m_recalculate_projection();
	}

	ortho_camera::ortho_camera(const glm::vec3& position, const glm::vec3& target) : camera(position, target) {
		m_left = m_top = -5.0f;
		m_right = m_bottom = 5.0f;
		m_near = 0.1f;
		m_far = 100.0f;
	...
	float ortho_camera::get_left() const {
		return m_left;
	}

	float ortho_camera::get_right() const {
		return m_right;
	}

	float ortho_camera::get_top() const {
		return m_top;
	}

	float ortho_camera::get_bottom() const {
		return m_bottom;
	}

	float ortho_camera::get_near() const {
		return m_near;
	}

	float ortho_camera::get_far() const {
		return m_far;
	}

	void ortho_camera::set_left(float left) {
		m_left = left;
		m_recalculate_projection();
	}

	void ortho_camera::set_right(float right) {
		m_right = right;
		m_recalculate_projection();
	}

	void ortho_camera::set_top(float top) {
		m_top = top;
		m_recalculate_projection();
	}

	void ortho_camera::set_bottom(float bottom) {
		m_bottom = bottom;
		m_recalculate_projection();
	}

	void ortho_camera::set_near(float near) {
		m_near = near;
		m_recalculate_projection();
	}

	void ortho_camera::set_far(float far) {
		m_far = far;
		m_recalculate_projection();
	}

	const glm::mat4x4& ortho_camera::get_projection_matrix() const {
		return m_projection;
	}

	const glm::mat4x4& ortho_camera::get_projection_inverse() const {
		return m_projection_inv;
	}

	void ortho_camera::video_mode_change(const video_mode_t& mode) {
		float oh = m_top - m_bottom;
		float ow = m_right - m_left;

		float nw = mode.get_buffer_width();
		float nh = mode.get_buffer_height();

		float old_aspect = glm::abs(ow / oh);
		float new_aspect = glm::abs(nw / nh);
		float ratio = new_aspect / old_aspect;

		m_left = ratio * m_left;
		m_right = ratio * m_right;

		m_recalculate_projection();
	}

	void ortho_camera::m_recalculate_projection() {
